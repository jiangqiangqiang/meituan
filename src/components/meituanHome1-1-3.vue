<!--正试账号：15903665494 12345678
测试账号：15903665494 12345678-->
<template>
  <div>
          ...
          <div class="partyRight">
            <div class="productListPage">
              ...
              <div v-if="shopListShow">
                <div class="logo"></div>
                <div class="menu">
                  <ul v-for="(menus,index) in typeList1.slice(0,8)" :key="index">
                    <li
                      :class="activeMenu==menus?'menuActive':null" @click="changeTab2(menus,index)">
                      <span style="float: left;margin-left: 15px">{{index+4}}</span>
                      {{menus}}
                    </li>
                  </ul>
                  <ul>
              ...
              <div v-if="seeMoreShow">
                <div class="diologBox" style="z-index: 9999">
                  <div class="cartBox" style="padding: 15px">
                    <div style="text-align: right;cursor: pointer" @click="backFn">返回</div>
                    <div style="line-height: 26px;margin-left: 6px;margin-bottom: 6px">热门搜索</div>
                    <el-tag type="warning" v-for="(shopTypeLis,i) in typeList1" :key=i style="margin: 6px;cursor: pointer">{{shopTypeLis}}</el-tag>
                  </div>
                </div>
              </div>
              <div v-if="DetailListShow">
                ...
                <div class="menu" style="height: 293px;">
                  <ul>
                    <li
                      @click="upperOrDown('FlipUp')">
                      向上翻
                    </li>
                  </ul>
                  <ul class="foodList" v-for="(menus,index) in dishes1.food_spu_tags.slice(food_spu_tagsStart,food_spu_tagsEnd)" :key="index">
                    <li
                      :class="activeMenu1==menus.name?'menuActive':null" :key="index"
                      @click="changeTab3(menus.name,index)" ref="activeMenu1">
                      <span style="float: left;margin-left: 15px">{{index+8}}</span>
                      {{menus.name}}
                    </li>
                  </ul>
                  <ul>
                    <li
                      @click="upperOrDown('FlipDown')">
                      向下翻
                    </li>
                  </ul>
                </div>
              ...
              <div class="help">
                <el-button type="text" @click="helpFn" style="color: rgba(246, 83, 2, 1)">语音助手</el-button>
              </div>
    ...
    methods: {
      ...
      websocketonmessage(e) {
        ...
        if (redata.msgType == "audio") {
          ...
          var obj = this.$store.getters.getShopImfByBotId(redata.botId);
          if(redata.data[redata.msgType] == "More"){
            if(obj.shopListShow){
              self.$store.dispatch("changObjByBotIdAndObj", {
                "botId": redata.botId,
                "obj": {seeMoreShow: true}
              });
              var obj3 = this.$store.getters.getShopImfByBotId(redata.botId);
              if (this.botId == redata.botId) {
                let userDataStatus = {
                  seeMoreShow: this.seeMoreShow, //订单预览
                } = obj3;
              }
              return
            }
          }
          if(redata.data[redata.msgType] == "FlipDown"){
            let self = this;

            self.$store.dispatch("updateFood_spu_tags", {
              "botId": redata.botId,
              "type": "down"
            });
            self.getUserDataStatus(redata.botId);
            var obj1 = this.$store.getters.getShopImfByBotId(redata.botId);
            var dishesChange=obj1.dishes1.food_spu_tags.slice(obj1.food_spu_tagsStart,obj1.food_spu_tagsEnd)[0]
            this.$store.dispatch("changObjByBotIdAndObj", {
              "botId": this.botId,
              "obj": {
                "dishesMenuIndex": 0,
                "activeMenu1": dishesChange.name,
                "shopDishesPageTotal": Math.ceil(dishesChange.spus.length / 3),
                "shopDishesPage": 1,
                "dishes22": this.pageSlice(1, 3, dishesChange.spus)
              }
            });
          ...
          if(redata.data[redata.msgType] == "FlipUp"){
            let self = this;
            self.$store.dispatch("updateFood_spu_tags", {
              "botId": redata.botId,
              "type": "upper"
          ...
          if (redata.data[redata.msgType] == "Fail") {
            if (obj.DetailListShow) {
              ...
              if(obj.seeMoreShow){
                self.$store.dispatch("changObjByBotIdAndObj", {
                  "botId": redata.botId,
                  "obj": {seeMoreShow: false,radio3:""}
                });
                var obj3 = this.$store.getters.getShopImfByBotId(redata.botId);
                if (this.botId == redata.botId) {
                  let userDataStatus = {
                    seeMoreShow: this.seeMoreShow,
                    radio3: this.radio3
                  } = obj3;
                }
                return
          ...
          if (redata.data[redata.msgType] == "Return") {
            ...
            if(obj.seeMoreShow){
              self.$store.dispatch("changObjByBotIdAndObj", {"botId": redata.botId, "obj": {seeMoreShow: false}});
              var obj3 = this.$store.getters.getShopImfByBotId(redata.botId);
              if (this.botId == redata.botId) {
                let userDataStatus = {
                  seeMoreShow: this.seeMoreShow
          ...
          if (redata.data[redata.msgType] == "PageUp") {
            ...
            if (obj.DetailListShow) {
              if (redata.data.data.index >= 8) {
                var dishesChange=obj.dishes1.food_spu_tags.slice(obj.food_spu_tagsStart,obj.food_spu_tagsEnd)[redata.data.data.index-8]
                this.$store.dispatch("changObjByBotIdAndObj", {
                  "botId": this.botId,
                  "obj": {
                    "dishesMenuIndex": redata.data.data.index-8,
                    "activeMenu1": dishesChange.name,
                    "shopDishesPageTotal": Math.ceil(dishesChange.spus.length / 3),
                    "shopDishesPage": 1,
                    "dishes22": this.pageSlice(1, 3, dishesChange.spus)
              ...
              if(redata.data.data.index<4){
                ...
                return
              }else if(redata.data.data.index<12){
                for (var i=0;i<obj.typeList1.length;i++){
                  if(i==redata.data.data.index-4){
                    this.$store.dispatch("changObjByBotIdAndObj", {
                      "botId": this.botId,
                      "obj": {radio3: obj.typeList1[i], shopPage: 1}
                    });
                  }
                }
                obj.typeList1
                this.$store.dispatch("changObjByBotIdAndObj", {
                  "botId": this.botId,
                  "obj": {shopMenuIndex: redata.data.data.index-4}
                });
                var obj3 = this.$store.getters.getShopImfByBotId(this.botId);
                let userDataStatus = {
                  activeMenu: this.activeMenu,
      ...
      getUserDataStatus(botId) {
        if (botId == this.botId) {
          ...
          let userDataStatus = {
            ...
            food_spu_tagsStart:this.food_spu_tagsStart,
            food_spu_tagsEnd:this.food_spu_tagsEnd
          } = obj
        }
      },
      toDetailShop1(index) {
